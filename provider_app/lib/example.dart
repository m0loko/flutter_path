...
class Complex {
  final int valueOne;
  final int valueTwo;
  Complex({required this.valueOne, required this.valueTwo});

  Complex copyWith({int? valueOne, int? valueTwo}) {
    return Complex(
...
class Model {
  ...
  Model({required this.one, required this.two, required this.complex});

  Model copyWith({int? one, int? two, Complex? complex}) {
    return Model(
      one: one ?? this.one,
      two: two ?? this.two,
